...
Here RevGrowth, GrossMargin, and OpMargin are revenue growth, gross margin, and operating margin, and ROA is return on assets; LM$^+$, LM$^-$, LM$^{Unc}$, and LM$^{Lit}$ are the Loughran--McDonald positive, negative, uncertainty, and litigious sentiment counts (defined fully in Section~\ref{sec:data}), named here to establish notation before that section's data description.
...
To assess whether the four rubric dimensions capture distinct information, we computed their pairwise correlations across the full 3,350-pair corpus (full matrix in Table~\ref{tab:dimcorr} in the Appendix). Non-responsiveness and vagueness are the most strongly related pair ($r = 0.725$), and deflection correlates especially strongly with non-responsiveness specifically ($r = 0.712$) despite showing more moderate correlation with vagueness and hedging on average; this is worth noting given that ``deflection'' appears as a labeled dimension in both our LLM rubric and the parallel human validation rubric, which may modestly inflate convergence on that particular dimension relative to a fully independent instrument. Hedging is the most distinct dimension, correlating least with the other three, and its relative independence from the other three dimensions, which themselves cluster together, suggests that a future two-factor representation (a ``directness'' factor spanning non-responsiveness, vagueness, and deflection, plus a separate hedging factor) might describe the construct more parsimoniously than a flat four-way average; we leave formal factor-analytic validation of this to future work.
...
\caption{SHAP feature importance for the best-performing XGBoost specification (Config A), ranking accounting fundamentals by mean absolute contribution to predictions of earnings-surprise direction. Evasion measures are not included in Config A; their absence from Configs B--D's top predictors (not shown) is consistent with the cross-validated null result reported in Section~\ref{sec:robustness}.}
\label{fig:shap}
\end{figure}
