...
All code, data pipeline scripts, and results underlying this paper are publicly available at \url{https://github.com/kunalchevuri/question-evasion-detection-LLM-framework_research} (commit \texttt{2e5bd18}). All data sources used (SEC EDGAR filings, Yahoo Finance price data, SEC XBRL structured data, the Loughran-McDonald dictionary) are free and publicly accessible. The repository README provides step-by-step reproduction instructions, with \texttt{final\_verification.py} as the primary entry point for reproducing all headline statistics reported in this paper.
