...
The judge receives the rubric in Figure~\ref{fig:prompt} as its system prompt and the question-response pair as the user turn, and returns the four integer ratings, a primary evasion type, and a one-sentence rationale as a single JSON object. Transcript-level measures aggregate across all pairs in $t$: mean evasion $\bar{E}_t$, within-transcript variance $\sigma^2_{E,t}$, and maximum pair-level score $E_t^{max}$, the last capturing acute evasion on a single salient question even when overall tone is direct.
