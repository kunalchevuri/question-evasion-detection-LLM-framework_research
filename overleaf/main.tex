...
\begin{enumerate}
\item \textbf{A continuous, interpretable measure.} Rather than a single categorical label, the framework returns a graded score decomposed into four named dimensions, so a reader can see not only how evasive a response was but in what way. It is training-free: it requires no labelled examples in the target domain, which is what makes it deployable in settings where no annotated corpus exists.
\item \textbf{A public, reproducible pipeline.} Every stage---retrieval from EDGAR, parsing, scoring prompts, evaluation, and the figures in this paper---is released as runnable code against freely available sources, together with measured cost and throughput so that reuse can be budgeted rather than guessed at.
...
\begin{table}[t]
\centering
\caption{Illustrative Application: Out-of-Sample AUROC by Configuration, with 5-Fold Cross-Validated Means}
\label{tab:auroc}
\begin{tabular}{lrr}
\toprule
...
All code, data pipeline scripts, and results underlying this paper are publicly available at \url{https://github.com/kunalchevuri/question-evasion-detection-LLM-framework_research} (commit \texttt{1c1bf17}). All data sources used (SEC EDGAR filings, Yahoo Finance price data, SEC XBRL structured data, the Loughran-McDonald dictionary) are free and publicly accessible. The repository README provides step-by-step reproduction instructions, with \texttt{final\_verification.py} as the primary entry point for reproducing all headline statistics reported in this paper.
