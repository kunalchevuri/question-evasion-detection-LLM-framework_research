...
\title{Measuring Evasion in Institutional Question-Answering: A Training-Free LLM-as-Judge Framework with Cross-Domain Validation}

\author{
\IEEEauthorblockN{Kunal Chevuri}
\IEEEauthorblockA{Prosper High School\\Prosper, TX\\chevukun000@k12.prosper-isd.net}
\and
\IEEEauthorblockN{Ayaan Sheikh}
\IEEEauthorblockA{Prosper High School\\Prosper, TX\\sheikaya000@k12.prosper-isd.net}
\and
...
\begin{abstract}
Institutional settings routinely place decision-makers in live, unscripted question-and-answer exchanges (congressional hearings, regulatory proceedings, central bank press conferences, corporate earnings calls) where a response may substantively engage with what was asked or evade it. We present a training-free framework that uses a large language model (LLM) as a structured judge to measure evasiveness on a continuous, interpretable scale, scoring each exchange along four dimensions: non-responsiveness, vagueness, deflection, and hedging. The framework requires no labeled data in the target domain and runs at $\$0.0038$ per exchange. We apply it to 3,350 question-response pairs from 285 earnings call transcripts filed with the U.S. Securities and Exchange Commission (SEC) and validate it three ways. Against blind annotation by two raters working without access to model output, on a stratified 75-pair sample, categorical agreement is modest but continuous agreement is strong (Pearson $r = 0.777$ to $0.803$), a pattern we show is characteristic of the construct rather than of our annotators. Against two external corpora scored blind and without tuning, the measure orders both label sets correctly: Spearman $\rho = 0.615$ on a large earnings-call benchmark and $\rho = 0.398$ on human-labeled political interviews, with one-vs-rest area under the receiver operating characteristic curve (AUROC) of $0.856$ and $0.741$ for the most-evasive class. The cross-domain result also locates the framework's limit: rank ordering transfers between domains, but absolute calibration does not, so thresholds must be re-derived per setting. Masking every company, ticker, executive, analyst, product, and place reference leaves scores essentially unchanged ($r = 0.861$), indicating the measure tracks the text of the exchange rather than recall of the underlying call. As a pre-specified downstream test we ask whether transcript-level evasion predicts market reaction to the subsequent earnings announcement; in 155 firm-quarters it does not, and we report that null as a boundary condition, reconciled against larger-sample findings via an explicit power analysis. The pipeline, prompts, and evaluation code are public.
\end{abstract}

\begin{IEEEkeywords}
large language models, LLM-as-judge, benchmark evaluation, cross-domain generalization, automated model evaluation, institutional question-answering, natural language processing, textual analysis, earnings calls
\end{IEEEkeywords}

\section{Introduction}

Many important institutional settings share a common structure: a decision-maker faces live, unscripted questioning from an interlocutor with the standing to demand a real answer (an analyst on an earnings call, a journalist at a press conference, a legislator in a hearing), and the responses given, or not given, carry information. Distinguishing a substantive answer from an evasive one is intuitive to a human listener but has proven difficult to measure systematically at scale. This paper introduces a tool for exactly that task: a structured large language model (LLM) judge that scores the evasiveness of a response along four interpretable dimensions (non-responsiveness, vagueness, deflection, and hedging), validated against blind human judgment and built entirely from public data and open methodology.

We develop and validate this framework using corporate earnings calls as our testbed, given the availability of full, unscripted question-and-answer transcripts filed publicly with the U.S. Securities and Exchange Commission (SEC). We apply our LLM judge (\texttt{claude-sonnet-4-6}, temperature 0) to 3,350 question-response pairs drawn from 285 transcripts across 49 companies and the 2015--2023 period, and validate its output against blind human annotation on a stratified sample of 75 pairs. While human raters do not always agree with each other or the model on precise categorical labels (a genuinely subjective task), agreement on the underlying continuous evasion scale is strong, indicating the framework captures real signal. We then take the same unmodified judge to two corpora assembled by other researchers, one in the same domain and one in political interviews, to test whether the measure holds up outside data we built ourselves.
...
\begin{enumerate}
\item \textbf{A continuous, interpretable measure.} Rather than a single categorical label, the framework returns a graded score decomposed into four named dimensions, so a reader can see not only how evasive a response was but in what way. It is training-free: it requires no labeled examples in the target domain, which is what makes it deployable in settings where no annotated corpus exists.
\item \textbf{A public, reproducible pipeline.} Every stage of the pipeline (retrieval from the Electronic Data Gathering, Analysis, and Retrieval (EDGAR) system, parsing, scoring prompts, evaluation, and the figures in this paper) is released as runnable code against freely available sources, together with measured cost and throughput so that reuse can be budgeted rather than guessed at.
\item \textbf{External and cross-domain validation.} We evaluate the unmodified judge against two independent corpora it was never tuned on: a large earnings-call benchmark and a human-annotated corpus of political interviews. The measure orders both correctly. The comparison also establishes where it stops working: rank ordering survives a change of domain, absolute calibration does not, which we regard as a more useful result than a single headline score.
\item \textbf{An honest, pre-specified downstream test.} We state a directional hypothesis drawn from voluntary disclosure theory, test it, and report that it fails. The null is stress-tested rather than discarded, and reconciled against larger-sample findings through an explicit power analysis.
\end{enumerate}

A reader should weigh the framework's demonstrated reliability independently of whether this particular financial hypothesis is supported.
...
\textbf{Earnings call linguistics and closely related recent work.} Larcker and Zakolyukina~\cite{larcker2012detecting} link deceptive or evasive linguistic patterns in earnings call narratives to subsequent financial misstatements; Jiang et al.~\cite{jiang2019manager} aggregate the textual tone of corporate financial disclosures into a manager sentiment index that negatively predicts aggregate stock returns; and Keith and Stent~\cite{keith2019modeling} model the pragmatics and semantics of analyst question framing during calls. Two recent efforts are closely related to our approach and warrant direct engagement. Ma et al.~\cite{ma2026evasionbench} introduce EvasionBench, a large-scale benchmark for managerial evasion in earnings-call Q\&A: 22.7 million raw pairs extracted from S\&P Capital IQ transcripts, filtered down to an 84,000-pair training set and a 1,000-pair human-labeled gold test set, built on a three-level taxonomy with multi-model-consensus annotation, achieving substantially higher human agreement (their reported $\kappa = 0.835$) than we observe here; their fine-tuned Eva-4B model attains 84.9\% Macro-F1 on that gold set. The gold split is not present in the corpus distributed on Hugging Face at the time of writing: those rows carry only labels generated by their Eva-4B-V2 model, although the paper states that a human-labeled gold set is released, which constrains how external evaluation against it can be interpreted, as we discuss in Section~\ref{sec:results}. Our approach differs in three ways: we use continuous, multi-dimensional, interpretable scoring rather than a single three-way label; we pair the measure with an explicit downstream market-reaction test; and we report a null result transparently rather than only the validation exercise. The higher agreement EvasionBench reports is a useful benchmark against which to read our own more modest categorical $\kappa$, and we view rubric design (particularly around explicit anchoring examples for each category) as a likely contributor to that gap. Separately, Thomas et al.~\cite{thomas2024never} introduce a two-level clarity and evasion taxonomy for political interview question-answering (the CLARITY/QEvasion line, since formalized as SemEval-2026 Task 6~\cite{thomas2026clarity}, which drew 124 registered teams and in which the best system reached 0.89 Macro-F1 at the clarity level while the top fine-grained evasion-type system reached only 0.68), which is the natural language processing (NLP) lineage for our stated goal of generalizing beyond corporate disclosure to interviews and testimony; we evaluate against both corpora directly in Section~\ref{sec:results}. Two further 2025 contributions bracket our own. Al Nuaimi et al.~\cite{alnuaimi2025evasive} propose a three-level psychological discourse taxonomy for financial Q\&A (response type, equivocation form, and fine-grained tactic) together with lightweight baselines, sharing our interest in interpretable structure but stopping short of a downstream economic test. Bamber and Nappert~\cite{bamber2025nonanswers} approach the same phenomenon qualitatively, against a backdrop, which they take from Hollander et al.~\cite{hollander2010silence}, of roughly 60\% of Q\&A sessions containing at least one silence, arguing that answers and non-answers are codependent rather than a simple signal of withheld bad news, a caution we take seriously in interpreting our own null.

\textbf{Reconciling a null with large-sample evidence.} Work on conference-call disclosure reports that non-answers carry measurable economic consequences: investors read declined questions negatively~\cite{hollander2010silence}, and non-answers concentrate on performance-related questions~\cite{gow2021nonanswers}, while our own downstream test (Section~\ref{sec:robustness}) does not. These are compatible, and it is worth stating why before the result is read as a contradiction. Three differences separate the settings. The samples differ by roughly two orders of magnitude: our analytical panel comprises 155 firm-quarters, against the tens of thousands of pairs available to corpus-scale studies. The measures differ: we regress a transcript-level mean of a continuous composite, whereas the comparison work typically classifies individual exchanges and aggregates differently. And the horizons differ: we test a three-day cumulative abnormal return around the subsequent announcement, a window chosen for identification rather than for power against slow-moving effects. Our post-hoc power analysis makes the implication concrete: at $n = 155$ the design has 34.4\% power to detect a difference of 0.10 in the area under the receiver operating characteristic curve (AUROC) and 66.4\% at 0.15. A null here therefore rules out a moderate-to-large effect in this sample; it is not evidence against the small effects that large samples are built to detect.
...
\textbf{Voluntary disclosure theory.} For our illustrative financial application, we draw on voluntary disclosure theory~\cite{dye1985disclosure,verrecchia2001essays}: managers with private information strategically withhold unfavorable disclosures when doing so is not fully unraveled by rational markets. This connects to a finance literature examining disclosure behavior around earnings communications directly. Hollander et al.~\cite{hollander2010silence} document that managers strategically decline to answer analyst questions during conference calls and that investors interpret such silence negatively. Most directly relevant, Gow et al.~\cite{gow2021nonanswers} construct a linguistic-analysis measure of ``non-answers'' during conference calls, finding roughly 11\% of analyst questions receive non-answers and that performance-related questions, and those with a negative tone, greater uncertainty, or greater complexity, are especially likely to trigger them; our LLM-based approach can be read as an alternative, more flexible operationalization of the same underlying construct they measure with hand-crafted linguistic rules. Their 11\% non-answer rate and our 2.84\% high-evasion rate (pair-level score $\geq 60$) are not directly comparable, since the two measures use different underlying constructs and thresholds, but the gap is broadly consistent with our continuous score capturing a wider range of partial engagement than a binary non-answer label, with only the most extreme cases crossing our higher threshold. Barcellos~\cite{barcellos2025overcoming} examines how investor suspicion moderates the market's ability to detect deceptive evasion tactics in earnings calls. Together these motivate our hypothesis that evasive responses may signal withheld unfavorable information, tested in Section~\ref{sec:results} and stress-tested further in Section~\ref{sec:robustness}.
...
\begin{figure*}[t]
\centering
\begin{Verbatim}[frame=single,framesep=7pt,fontsize=\footnotesize,xleftmargin=0pt,xrightmargin=0pt]
SYSTEM: You are an expert analyst of corporate earnings call communications, evaluating whether a
management response evades or directly answers an analyst's question.

Score the response on four dimensions, each an integer 1-5 (5 = most evasive):
1. non_responsiveness: Does the response fail to address what was specifically asked?
   (1 = fully addresses the question, 5 = completely ignores/talks past it)
2. vagueness: Does the response avoid concrete, verifiable specifics (numbers, dates, named
   initiatives) in favor of generic language? (1 = highly specific and concrete,
   5 = entirely vague/generic)
3. deflection: Does the response pivot to unrelated talking points, unprompted narratives, or a
   different topic than what was asked? (1 = stays on topic, 5 = heavy redirection)
4. hedging: Does the response explicitly decline to answer, hedge with disclaimers, or defer
   ("we don't guide on that," "let's take this offline")? (1 = a clear, committed answer,
   5 = explicit non-answer/refusal)

Also pick a primary_evasion_type: one of "none", "non_responsive", "vague", "deflection",
"hedging" (the dominant pattern, or "none" if the response is not evasive).

Respond with ONLY a single-line JSON object, no markdown formatting, no commentary before or after:
{"non_responsiveness": <1-5>, "vagueness": <1-5>, "deflection": <1-5>, "hedging": <1-5>,
 "primary_evasion_type": "<type>", "rationale": "<one sentence>"}
...
USER:   ANALYST QUESTION:
        ...
        {response}
\end{Verbatim}
\caption{The scoring instrument, reproduced from \texttt{src/judge.py}; only line wrapping and the
\texttt{SYSTEM}/\texttt{USER} labels have been added. Applied with \texttt{claude-sonnet-4-6} at
temperature 0 and \texttt{max\_tokens} 300. No in-context examples and no target-domain labels are
supplied at any point, which is what makes the framework training-free and what allows the identical
prompt to be carried unchanged to the two external corpora in Section~\ref{sec:results}.}
\label{fig:prompt}
\end{figure*}

The judge receives the rubric in Figure~\ref{fig:prompt} as its system prompt and the question-response pair as the user turn, and returns the four integer ratings, a primary evasion type, and a one-sentence rationale as a single JavaScript Object Notation (JSON) object. Transcript-level measures aggregate across all pairs in $t$: mean evasion $\bar{E}_t$, within-transcript variance $\sigma^2_{E,t}$, and maximum pair-level score $E_t^{max}$, the last capturing acute evasion on a single salient question even when overall tone is direct.
...
\begin{tabular}{lrrr}
\toprule
 & \multicolumn{2}{c}{Cohen's $\kappa$} & \\
\cmidrule(lr){2-3}
Rater pair & Unwt. & Quad. & Pearson $r$ \\
\midrule
Human1 vs.\ Human2 & 0.237 & 0.382 & 0.587 \\
LLM vs.\ Human1 & 0.272 & 0.448 & 0.777 \\
LLM vs.\ Human2 & 0.450 & 0.609 & 0.803 \\
\midrule
\multicolumn{4}{l}{\emph{Corpus}} \\
Total scored Q\&A pairs & \multicolumn{3}{r}{3,350} \\
Mean evasion, pair-level ($n$=3,350) & \multicolumn{3}{r}{27.50 (SD 15.58)} \\
Mean evasion, transcript-level ($n$=155) & \multicolumn{3}{r}{28.9 (SD 8.1)} \\
\bottomrule
\end{tabular}
\end{table}
...
Macro-F1, fitted (held out) & 0.595 & 0.447 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[t]
\centering
\caption{Illustrative Application: Out-of-Sample AUROC by Configuration, with 5-Fold Cross-Validated Means. LR is logistic regression; XGB is XGBoost.}
\label{tab:auroc}
\begin{tabular}{lrr}
\toprule
Configuration & LR & XGB \\
\midrule
\multicolumn{3}{l}{\emph{Chronological hold-out}} \\
...
\midrule
\multicolumn{3}{l}{\emph{5-fold cross-validation, mean (SD)}} \\
B & 0.508 (0.165) & 0.528 (0.084) \\
C & 0.508 (0.136) & 0.529 (0.075) \\
\bottomrule
\end{tabular}
\\[4pt]
...
We ran 5-fold stratified cross-validation on Configs B and C (Table~\ref{tab:auroc}) to characterize the null result rigorously rather than reporting a single specification. Mean AUROC is statistically indistinguishable across configurations for both model types, and fold-to-fold SD (0.075--0.165) dwarfs the single-split point estimate, confirming the result reflects sampling variance rather than a stable effect.

A further check addresses the imputation disclosed in Section~\ref{sec:methodology}: gross margin is observed for 109 of 155 panel observations and operating margin for 113, with the remainder filled from training-set medians. Restricting the panel to the 79 observations where both margins are genuinely observed leaves the null intact: no evasion measure approaches significance (Pearson $p = 0.43$ to $0.90$; Spearman $p = 0.48$ to $0.83$), so the result is not an artifact of imputation. Because the 2015--2023 sample spans a period of extreme, largely firm-independent volatility, we re-estimated all specifications excluding 2020 (25 observations); results are essentially unchanged ($r = 0.002$, $p = 0.982$, $n = 130$; AUROC shifts of 0.01 or less), so the null result is not a COVID-era artifact. A post-hoc power analysis indicates that at $n = 155$, this design has 7.0, 12.9, 34.4, and 66.4\% power to detect true AUROC differences of 0.02, 0.05, 0.10, and 0.15 respectively, none reaching conventional thresholds: the data rule out a moderate-to-large effect but remain underpowered for smaller ones, a limitation of sample size rather than of the framework itself. A SHapley Additive exPlanations analysis of the best XGBoost specification ranks return on assets as most influential, followed by revenue growth and the two margin measures, with evasion measures absent from that configuration entirely, consistent with the cross-validated null result.


Because the 155-observation panel spans only 32 firms with repeated observations per firm, standard errors that ignore this clustering may understate uncertainty. We re-estimated the Config C logistic specification with standard errors clustered by firm: the coefficient on mean evasion score remains outside conventional significance under both naive ($p = 0.083$) and firm-clustered ($p = 0.054$) standard errors. We state the direction of movement rather than gloss it: clustering moves the $p$-value toward significance, not away from it, and $0.054$ is marginal. The null therefore survives this check narrowly, and the weight of the evidence against a detectable effect rests on the cross-validation, imputation, and power analyses above rather than on this one.

The pairwise correlations among the four rubric dimensions are reported in Table~\ref{tab:dimcorr}. One caveat belongs here: ``deflection'' appears as a labeled dimension in both our LLM rubric and the parallel human validation rubric, which may modestly inflate convergence on that dimension relative to a fully independent instrument.
...
A related concern is that a continuous evasiveness score might be little more than a proxy for response length, with terse answers scored evasive and long ones substantive. It is not. On the two external corpora of Section~\ref{sec:results}, raw response length carries almost no rank information about the external label: Spearman $\rho = -0.033$ ($p = 0.56$) on CLARITY and $+0.098$ ($p = 0.002$) on EvasionBench, against the judge's own $\rho = +0.398$ and $+0.615$ on the identical items. Length and judge score do correlate modestly on EvasionBench ($\rho = 0.304$) and negligibly on CLARITY ($\rho = 0.065$), which is expected and benign, since genuinely non-responsive answers tend to be shorter. What the comparison rules out is that verbosity alone reproduces the measure's external agreement.
...
Because the judge model was trained on public web text that plausibly includes some of these 2015--2023 SEC transcripts, a natural concern is training-data contamination: could the model's parametric memory of a company's subsequent performance, rather than the text presented to it, be tinting its evasion score? We consider this a lower-risk form of contamination than factual recall tasks, since the judge is asked to rate specific textual features of the presented question-response pair (directness, hedging language, deflection) rather than to recall or predict an outcome, and the prompt explicitly withholds market and outcome information (Section~\ref{sec:results}). We also test it directly rather than leaving it as a residual concern: Section~\ref{sec:results} reports scores recomputed after every company, ticker, executive, analyst, product, and place reference is masked, and finds them substantially unchanged. That result is what allows us to treat contamination as addressed rather than merely argued, subject to the interpretive caveat recorded there.

\textbf{Limitations.} We summarize the main limitations of this study explicitly. The primary judge is a single LLM (\texttt{claude-sonnet-4-6}); Section~\ref{sec:results}'s cross-model comparison and the prompt-robustness check above both suggest the construct generalizes across models and prompt wordings, though both checks were conducted on the same 75-pair subsample rather than the full corpus. Generalization beyond our own corpus is now evidenced rather than assumed, but with a stated limit: the external evaluations show that the measure's rank ordering transfers to a different domain while its absolute calibration does not, so cut points derived here should not be carried into a new setting unchanged. Human validation relies on two annotators who are also this paper's student authors, working blind to model output but drawn from a small, non-independent pool; a larger and more diverse annotator panel would strengthen this further. The financial application's panel is modest (155 observations, 32 firms) and firm concentration, while improved from an earlier iteration of this dataset, remains a relevant caveat; firm-clustered standard errors leave the null intact, though narrowly (Section~\ref{sec:robustness}). The evasion score itself is compressed (16 of 17 possible pair-level values realized, with the transcript-level means used in the financial application clustered between roughly 20 and 35), which limits the amount of downstream variation available to any predictive model. The CAR measure used is market-adjusted rather than risk-adjusted, some earnings announcement dates are estimated rather than directly observed (flagged via \texttt{car\_date\_estimated}), and the realized company panel, drawn from EDGAR full-text search rather than a pre-specified sector universe, skews toward industrials and services rather than the technology and healthcare sectors originally targeted; readers should weigh the generalizability of the financial application, specifically, accordingly.

\textbf{Positioning relative to comparable published work.} The closest published antecedents to our financial application, Larcker and Zakolyukina~\cite{larcker2012detecting} and Gow et al.~\cite{gow2021nonanswers}, both appearing in the \emph{Journal of Accounting Research}, and Barcellos~\cite{barcellos2025overcoming}, in \emph{Management Science}, draw on far larger conference-call samples (hundreds to thousands of firm-quarters) than our 155-observation illustrative panel. We do not claim parity with those samples; rather, we position this paper's contribution as the validated measurement framework itself, with the financial application serving as a rigorously-reported test case rather than a claim of comparable statistical power. Extending the framework to a sample of that scale is the natural next step and the most direct path to a stronger financial contribution.

\section{Conclusion}

This paper introduces and validates a structured LLM-as-judge framework for measuring evasion in adversarial institutional question-and-answer exchanges, applied at scale to 3,350 real question-response pairs from 285 SEC-filed earnings call transcripts. The framework's evasion scores correlate meaningfully with human judgment on a continuous scale, even where categorical agreement is more modest, extending the LLM-as-judge paradigm from its established use in benchmarking generative model outputs to detecting strategic communicative behavior in real institutional settings. Validation is not confined to data we assembled: applied unmodified and label-blind to two corpora built by other researchers, the measure orders both label sets correctly, and masking every identifying reference leaves its scores substantially unchanged. As an illustrative test of a concrete downstream application, we examined whether evasiveness predicts subsequent market reactions to earnings announcements and found no significant relationship, a result stable under cross-validation and robust to excluding the COVID-19 period; a formal power analysis indicates this reflects the limits of this specific sample rather than definitive evidence against any relationship at any scale. We regard this as a successful demonstration of how the framework should be applied and evaluated rigorously, not as a verdict on its value.

We see the framework's more general contribution as extending well beyond corporate earnings calls: any adversarial institutional Q\&A setting (legislative testimony, regulatory hearings, press conferences by public officials) presents a natural application for a validated, scalable, human-interpretable measure of evasiveness. The political-interview evaluation makes that more than an assertion, while also marking the condition attached to it: what transfers across domains is the ordering, not the scale, so a new setting requires its own thresholds even though it does not require its own training data. Future work applying this framework to a substantially larger and more diverse financial sample would help determine whether the null result reported here reflects a genuine absence of relationship or the statistical limits of the present study.

\section*{Data and Code Availability}

All code, data pipeline scripts, and results underlying this paper are publicly available at \href{https://github.com/kunalchevuri/question-evasion-detection-LLM-framework_research}{\footnotesize\texttt{github.com/kunalchevuri/\allowbreak question-evasion-detection-LLM-framework\_research}} (commit \texttt{7a60b7a}). All data sources used (SEC EDGAR filings, Yahoo Finance price data, SEC XBRL structured data, the Loughran-McDonald dictionary) are free and publicly accessible. The repository README provides step-by-step reproduction instructions, with \texttt{final\_verification.py} as the primary entry point for reproducing all headline statistics reported in this paper.
